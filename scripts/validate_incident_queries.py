...
def _registry(schemas: dict[str, dict]) -> Registry:
    """Explicit registry keyed by published $id identities only.

    No file-path aliases: every reference must resolve exactly as published,
    the way a real consumer receives the schemas.
    """
    resources: list[tuple[str, Resource]] = []
    for schema in schemas.values():
        resources.append((schema.get("$id", ""), Resource.from_contents(schema)))
...
def _resolve_pointer(document: Any, pointer: str) -> bool:
    """Resolve a local JSON Pointer (RFC 6901) against the original document.

    Supports the empty pointer (whole document), `/`-separated tokens and the
    `~0`/`~1` escapes. Returns whether the target exists; no aliases, no
    repairs, no document mutation.
    """
    if pointer == "":
        return True
...
def check_openapi(registry: dict[str, dict]) -> list[str]:
    ...
    for ref in _iter_refs(doc):
        if ref.startswith("#"):
            if not _resolve_pointer(doc, ref[1:]):
                errors.append(f"openapi references missing local target '{ref}'")
...
def check_examples(registry: dict[str, dict]) -> list[str]:
    ...
    validators = {
        name: Draft202012Validator(
            schema, registry=_registry(registry), format_checker=FormatChecker()
        )  # noqa: E501
        for name, schema in registry.items()
    ...
    for filename, schema_name in POSITIVE.items():
        ...
        try:
            ...
            errors.append(f"positive example '{filename}' cannot be validated: {error}")
            continue
    ...
    for filename in sorted(ERROR_EXAMPLES):
        ...
        try:
            ...
            errors.append(f"error example '{filename}' cannot be validated: {error}")
            continue
        for problem in problems:
            location = "/".join(str(part) for part in problem.path) or "<root>"
