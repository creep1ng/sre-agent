...
Every `AuditEvent` SHALL contain event ID/time, server `request_id`, canonical operation/action, processing `stage`, outcome and safe error/reason code when applicable, redaction status/policy version, and safe technical correlation when present. `stage` SHALL be one of `validation|authentication|authorization|routing|upstream|response|audit` and SHALL determine which additional dimensions are valid.

Before authentication succeeds, PrincipalContext SHALL be absent. A 401 event MAY include Principal and safe credential ID only when each was resolved safely; otherwise each SHALL be absent. Raw credentials and `Authorization` SHALL never serialize. After authentication succeeds, the authenticated Principal, PrincipalContext reference, and safe credential ID SHALL be present. Resource/ModelAlias and PolicyDecision SHALL be present only after authorization is evaluated and SHALL then be required. Concrete model, router, and inference provider SHALL be present only after resolution/execution. Untrusted request-body model or domain IDs MAY appear only as sanitized, explicitly untrusted input metadata and SHALL NOT be represented as resolved identity, Resource, ModelAlias, or PolicyDecision. When applicable, `incident_id`, `run_id`, `task_id`, and trace ID SHALL remain separate safe correlation dimensions. Upstream secrets and provider objects SHALL NOT serialize.
