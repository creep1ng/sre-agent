...
| Stage-aware audit | Every safe event always has event/time, request ID, operation/action, processing stage, outcome/error code, redaction status/version, and available safe technical correlation. Identity, authorization, and routing dimensions become required only after their stages succeed or are evaluated; untrusted body IDs never masquerade as resolved authority. | A single all-fields-required shape would force fabricated Principal/decision data for pre-auth 401/422 outcomes. |
