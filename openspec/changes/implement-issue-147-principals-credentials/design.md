...
| Audit shape | Control events use `stage=authorization`, metadata-only identity/resource refs, `content_state=absent`, no routing leakage. New operations extend the closed vocabulary; 1.3.0 denial-cause rules unchanged. | A distinct administrative resource type keeps control-plane evidence separate from governed-LLM resources; raw IDs in audit break ADR-005 domain separation. |
