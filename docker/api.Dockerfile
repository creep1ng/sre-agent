...
COPY .github ./.github
COPY docker ./docker
# The demo manifest and its overlay: tests assert the published host ports.
COPY demo ./demo
COPY .dockerignore compose.yaml compose.mcp.yaml compose.demo.yaml README.md .importlinter playwright.config.js playwright.production.config.js ./
