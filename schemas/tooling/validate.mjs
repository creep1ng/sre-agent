...
if (!(args.length === 0 || (args.length === 1 && !args[0].startsWith("-")) || scoped)) {
  console.error("Usage: node validate.mjs [fixture-directory | --scope identity|model-resource|policy|shared|audit|redaction-success|redaction-failure|governance|idempotency|credentials|bootstrap|control|responses|responses-boundary|openrouter-metadata|responses-errors]"); process.exitCode = 2;
} else try {
  if (scoped && args[1] === "governance") { const result = await validateGovernance(new URL("../releases/1.0.0/", import.meta.url), new URL("../adrs/", import.meta.url)); console.log(`Validated ${result.authorities} authorities, ${result.placements} ownership placements, and ${result.adrs} accepted ADRs.`); }
