...
if (!(args.length === 0 || (args.length === 1 && !args[0].startsWith("-")) || scoped)) {
  console.error("Usage: node validate.mjs [fixture-directory | --scope identity|model-resource|policy|shared|audit|redaction-success|redaction-failure|governance|idempotency|credentials|bootstrap|control|responses]"); process.exitCode = 2;
} else try {
