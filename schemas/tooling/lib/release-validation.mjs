...
const baseline = (version) => version === "1.0.0" ? { previous_release: null, previous_major: null, compatibility: "initial-publication" } : version === "1.1.0" ? { previous_release: "1.0.0", previous_major: "1.0.0", compatibility: "additive" } : null;
...
export async function validateCompatibility(previousVersion = "1.0.0", version = "1.1.0") {
  const previous = await loadReleaseDirectory(releaseRoot(previousVersion), "all"), current = await loadReleaseDirectory(releaseRoot(version), "all"), positives = previous.fixtures.filter(({ status }) => status === "positive"), advance = (target) => target.replace(new RegExp(`:${previousVersion.replaceAll(".", "\\.")}$`), `:${version}`);
  try { validateFixtures(current.schemas, positives.map((fixture) => ({ ...fixture, target: advance(fixture.target), version }))); validateExamples(current.schemas, previous.examples.map((example) => ({ ...example, target: advance(example.target) }))); } catch (error) { throw new Error(`Additive compatibility failed: ${error.message}`); }
  return { previous_release: previousVersion, current_release: version, positive_fixtures: positives.length, examples: previous.examples.length, status: "passed" };
}
...
export async function generateRelease(version = DEFAULT_VERSION) {
  ...
  if (version !== DEFAULT_VERSION) { const compatibility = await validateCompatibility(DEFAULT_VERSION, version); await writeImmutable(join(root, "conformance/compatibility.json"), compatibility); results.push({ consumer: "cross-version", action: "additive-compatibility", fixture: "conformance/compatibility.json", status: "passed" }); }
...
export async function validateRelease(version = DEFAULT_VERSION) {
  const root = releaseRoot(version), results = await runConformance(root); await validateGovernance(pathToFileURL(`${root}/`), pathToFileURL(`${adrs}/`), version);
  if (version !== DEFAULT_VERSION) { const expected = await validateCompatibility(DEFAULT_VERSION, version), actual = JSON.parse(await readFile(join(root, "conformance/compatibility.json"))); assertImmutableManifest(actual, expected); results.push({ consumer: "cross-version", action: "additive-compatibility", fixture: "conformance/compatibility.json", status: "passed" }); }
