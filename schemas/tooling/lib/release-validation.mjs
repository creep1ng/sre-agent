...
const tooling = fileURLToPath(new URL("../", import.meta.url)), schemas = resolve(tooling, ".."), release = join(schemas, "releases/1.0.0"), adrs = join(schemas, "adrs");
...
async function assertPinnedArtifact(root, fixture) { const manifest = await optional(join(root, "manifest.yaml"), parse); if (!manifest) return; const path = `releases/1.0.0/${fixture}`, record = [...(manifest.inventory?.fixtures ?? []), ...(manifest.inventory?.examples ?? [])].find((item) => item.path === path); if (!record || record.sha256 !== await fileHash(join(root, fixture))) throw new Error(`Obligation fixture ${fixture} does not match its immutable manifest hash`); }
...
const ACTIONS = {
  ...
  "execution-openapi": async (root) => { await runReleaseOpenapi("responses", join(tooling, ".tmp/responses.yaml")); await assertFutureFastapi(pathToFileURL(`${resolve(root)}/`)); },
  "control-openapi": () => runReleaseOpenapi("control-plane", join(tooling, ".tmp/control-plane.yaml"))
};

export async function validateCoverage(root = release) {
  const suite = await parsed(join(root, "conformance/suite.yaml")), consumers = await parsed(join(root, "conformance/consumers.yaml"));
  ...
  if (suite.contract_version !== "1.0.0" || consumers.contract_version !== "1.0.0" || canonical(consumers.prohibited_authorities) !== canonical(["framework_dto", "orm_model", "database_model", "provider_sdk_type"])) throw new Error("Conformance metadata is not version-aligned and authority-closed");
...
export async function runConsumer(consumer, root = release) { const { suite } = await validateCoverage(root), policy = POLICY[consumer]; if (!policy) throw new Error(`Unknown consumer: ${consumer}`); const item = suite.obligations.find(({ id }) => id === policy[1]); await validateObligationFixture(root, item); await ACTIONS[policy[2]](root); return { consumer, action: policy[2], fixture: item.fixture, status: "passed" }; }
...
async function inventory(includeEvidence = true) {
  const releaseFiles = await walk(release), records = { openapi: [], schemas: [], examples: [], fixtures: [], adrs: [], conformance: [] };
  for (const local of releaseFiles) { if (local === "manifest.yaml" || (!includeEvidence && local === "conformance/evidence.json")) continue; const path = join(release, local), record = { path: `releases/1.0.0/${local}`, sha256: await fileHash(path) };
  ...
  for (const local of (await walk(adrs)).filter((name) => /^ADR-\d{3}-.*\.md$/.test(name))) records.adrs.push({ path: `adrs/${local}`, sha256: await fileHash(join(adrs, local)) }); return records;
}
async function semanticHashes() { const outputs = [join(tooling, ".tmp/evidence-control.yaml"), join(tooling, ".tmp/evidence-responses.yaml")]; try { await Promise.all([runReleaseOpenapi("control-plane", outputs[0]), runReleaseOpenapi("responses", outputs[1])]); const documents = await Promise.all(outputs.map(readContractFile)); return { control_plane: digest(canonical(normalizeOpenapi(documents[0]))), responses: digest(canonical(normalizeOpenapi(documents[1]))), combined_projection: digest(canonical(combineOpenapi(...documents))) }; } finally { await Promise.all(outputs.map((path) => rm(path, { force: true }))); } }
async function evidenceObject(results) { const packageJson = JSON.parse(await readFile(join(tooling, "package.json"))), inputs = await inventory(false), semantics = await semanticHashes(); semantics.adrs = digest(canonical(inputs.adrs)); semantics.fixtures = digest(canonical(inputs.fixtures)); return { contract_version: "1.0.0", inputs_sha256: digest(canonical(inputs)), semantics, results: [...results, { consumer: "governance", action: "ownership-and-adrs", status: "passed" }], toolchain: { node: packageJson.engines.node, ajv: packageJson.devDependencies.ajv, redocly: packageJson.devDependencies["@redocly/cli"], yaml: packageJson.devDependencies.yaml } }; }
async function manifestObject() { return { contract_version: "1.0.0", status: "immutable", baseline: { previous_release: null, previous_major: null, compatibility: "initial-publication" }, dialects: { json_schema: "https://json-schema.org/draft/2020-12/schema", openapi: "3.1.0" }, inventory: await inventory(true) }; }
export function assertImmutableManifest(existing, candidate) { if (canonical(existing) !== canonical(candidate)) throw new Error("Release 1.0.0 is immutable; publish a new version instead of rewriting it"); }
export function assertReleaseMetadata(manifest) { exact(manifest, ["contract_version", "status", "baseline", "dialects", "inventory"], "Release manifest"); exact(manifest.baseline, ["previous_release", "previous_major", "compatibility"], "Release baseline"); exact(manifest.dialects, ["json_schema", "openapi"], "Release dialects"); exact(manifest.inventory, ["openapi", "schemas", "examples", "fixtures", "adrs", "conformance"], "Release inventory"); if (Object.values(manifest.inventory).some((items) => !Array.isArray(items))) throw new Error("Manifest inventory categories must be arrays"); if (manifest.contract_version !== "1.0.0" || manifest.status !== "immutable" || canonical(manifest.baseline) !== canonical({ previous_release: null, previous_major: null, compatibility: "initial-publication" })) throw new Error("Manifest release baseline is invalid"); if (canonical(manifest.dialects) !== canonical({ json_schema: "https://json-schema.org/draft/2020-12/schema", openapi: "3.1.0" })) throw new Error("Manifest release dialects are invalid"); if (manifest.inventory.schemas.some((item) => item.dialect !== manifest.dialects.json_schema || !item.id.endsWith(":1.0.0")) || manifest.inventory.openapi.some((item) => item.dialect !== manifest.dialects.openapi || item.api_major !== 1 || !SEMVER.test(item.version) || Number(item.version.split(".")[0]) !== item.api_major)) throw new Error("Manifest $id or API major is invalid"); }
...
export async function generateRelease() {
  const results = await runConformance(); await validateGovernance(new URL("../../releases/1.0.0/", import.meta.url), new URL("../../adrs/", import.meta.url));
  const evidence = await evidenceObject(results), evidencePath = join(release, "conformance/evidence.json"); await writeImmutable(evidencePath, evidence);
  const manifest = await manifestObject(), manifestPath = join(release, "manifest.yaml"); assertReleaseMetadata(manifest); await writeImmutable(manifestPath, manifest, { decode: parse, encode: (value) => stringify(value, { sortMapEntries: true, lineWidth: 0 }) }); return { artifacts: Object.values(manifest.inventory).flat().length, results: results.length + 1 };
}

export async function validateRelease() {
  const results = await runConformance(); await validateGovernance(new URL("../../releases/1.0.0/", import.meta.url), new URL("../../adrs/", import.meta.url));
  const manifest = parse(await readFile(join(release, "manifest.yaml"), "utf8")), expectedManifest = await manifestObject(); assertReleaseMetadata(manifest); assertImmutableManifest(manifest, expectedManifest);
  const evidence = JSON.parse(await readFile(join(release, "conformance/evidence.json"))), expectedEvidence = await evidenceObject(results); assertImmutableManifest(evidence, expectedEvidence); return { artifacts: Object.values(manifest.inventory).flat().length, results: evidence.results.length };
}

export async function writeProjectionFixtures() {
  if (await optional(join(release, "manifest.yaml"), parse)) throw new Error("Release 1.0.0 is immutable; projection fixtures cannot be regenerated"); const outputs = [join(tooling, ".tmp/projection-control.yaml"), join(tooling, ".tmp/projection-responses.yaml")]; await mkdir(dirname(outputs[0]), { recursive: true }); try { await Promise.all([runReleaseOpenapi("control-plane", outputs[0]), runReleaseOpenapi("responses", outputs[1])]); const match = { contract_version: "1.0.0", ...combineOpenapi(...await Promise.all(outputs.map(readContractFile))) }, missing = structuredClone(match), extra = structuredClone(match), first = Object.keys(missing.paths).sort()[0]; delete missing.paths[first]; extra.paths["/__future_drift__"] = { get: { parameters: [], security: { requirements: [], schemes: {} }, request: null, responses: { 200: { content: {} } } } };
    const files = [["positive/future-fastapi.match.projection.json", match], ["negative/future-fastapi.missing.projection.json", missing], ["negative/future-fastapi.extra.projection.json", extra]]; for (const [name, value] of files) await writeFile(join(release, "fixtures", name), `${canonical(value)}\n`); return files.map(([name]) => name);
