...
export async function assertFutureFastapi(root = new URL("../releases/1.0.0/", import.meta.url)) {
  const outputs = [temporary("future-control.yaml"), temporary("future-responses.yaml")];
  try {
    await Promise.all([runReleaseOpenapi("control-plane", outputs[0]), runReleaseOpenapi("responses", outputs[1])]);
