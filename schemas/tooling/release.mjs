...
const [operation, ...args] = process.argv.slice(2), version = args[0] === "--release" && /^(?:1\.0\.0|1\.1\.0|1\.2\.0|1\.3\.0|1\.4\.0)$/.test(args[1] ?? "") && args.length === 2 ? args[1] : null, consumer = args[0] === "--consumer" && args[1] && args.length === 2, coverage = args[0] === "--check" && args[1] === "coverage" && args.length === 2;
try {
  ...
  else { console.error("Usage: node release.mjs projection|evidence|validate --release 1.0.0|1.1.0|1.2.0|1.3.0|1.4.0 | validate-all | conformance --check coverage | conformance --consumer <id>"); process.exitCode = 2; }
