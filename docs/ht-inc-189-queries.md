...
`src/sre_agent/gateway/incidents.py` (`IncidentQueryService` + `incident_router`,
montado en `application.py`). Tests HTTP por superficie:
`tests/test_incident_query_detail.py`,
`tests/test_incident_query_timeline.py`; proyecciones puras en
`tests/test_incident_projections.py`; fakes compartidos en
`tests/query_testkit.py`.
...
Detalle reporta la versión del agregado persistido, verbatim. Snapshot
reporta la versión y el estado CUBIERTOS por el snapshot (históricos): tras
avanzar el incidente, detalle y snapshot difieren a propósito y el snapshot
nunca mezcla estado actual.
...
GET. `version`/`event_sequence` permiten a #36 detectar staleness. El bloque
`incident`/`run` del snapshot se proyecta SOLO desde `incident_state` y
`run_state` persistidos en el snapshot: un snapshot tomado en triage sigue
mostrando triage aunque el incidente haya avanzado (regresión en
`tests/test_incident_projections.py` y prueba de integración en
`tests/test_incident_query_timeline.py`).
...
pytest tests/test_incident_query_detail.py tests/test_incident_query_timeline.py -q
```

PG (lane obligatoria, mismo patrón que `tests/test_incident_persistence.py`):
la base efímera del proyecto vía `TEST_DATABASE_URL`
(por defecto `postgresql://postgres:postgres@127.0.0.1:55466/postgres`);
el fixture del módulo recrea el schema y migra desde cero. Una base
inaccesible o mal configurada FALLA en lugar de omitirse en silencio; no usar
DSNs personales hardcodeados. CA3 a nivel de persistencia está probado:
`test_snapshot_stays_historical_after_advance_and_recreate` persiste un
snapshot en triage, avanza a investigating, comprueba detail/timeline/snapshot
y relee idéntico tras recrear pools y servicio.
Demo HTTP contra stack: `docker compose up`, credencial con grant `run.read`
preparado de forma reproducible y acotada (una fila en `resources` para
`incident_workflow`/`incident-response` en estado activo más una fila en
`grants` con `action=run.read`, `effect=allow`, `status=active` para ese
principal; sin grants globales, deny por defecto en todo lo demás),
`curl -H "Authorization: Bearer ..."` a los tres endpoints.

## Limitaciones conocidas

- CA3 sobre HTTP empaquetado con reinicio (evidencia de aceptación de la
  issue) sigue pendiente de provisión de grants: hoy ningún `run.read` puede
  provisionarse porque `ck_resources_type` rechaza `incident_workflow` y el FK
  de `grants` exige la fila (`ERROR: new row for relation "resources"
  violates check constraint "ck_resources_type"`, reproducido contra una DB
  migrada). Proveer el grant pertenece a #184 (operaciones del plano de
  control) o a una decisión de seeds/migración; sin excepción, sin permisos
  globales y sin relajar constraints productivos.
