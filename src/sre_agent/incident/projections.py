...
def project_snapshot(snapshot: SnapshotRecord, workflow: IncidentWorkflow) -> dict[str, Any]:
    """Project the persisted historical snapshot, never the current aggregate.

    The nested incident/run documents come only from ``snapshot.incident_state``
    / ``snapshot.run_state``. ``version``/``event_sequence``/``created_at`` are
    the snapshot's own historical markers; the nested blocks report the covered
    aggregate version and that state's commit time, reusing the safe detail/run
    projections.
    """
    record = IncidentRecord(
        incident_id=snapshot.incident_id,
        state=snapshot.incident_state,
    ...
    return {
        ...
        "incident": project_detail(record, [run], workflow),
