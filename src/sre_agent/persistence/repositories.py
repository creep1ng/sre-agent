...
class CatalogRepository:
    """Project owner-authoritative MCP identities into the resource catalog."""
    ...
    async def project_mcp_server(self, server: MCPServer) -> ResourceCatalogEntry:
        ...
        if row is None:
            ...
            self._refresh_mcp_projection(row, server)
        await self._session.flush()
    ...
    def _refresh_mcp_projection(
        row: ResourceRow, resource: MCPServer | MCPTool, *, source_ref: str | None = None
    ) -> None:
        row.status = resource.status
        row.updated_at = resource.updated_at
        row.owner_id = resource.owner_id
        row.source = "mcp"
        row.source_ref = source_ref or f"mcp-owner/{resource.server_id}"
